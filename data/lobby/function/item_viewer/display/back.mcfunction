...
#上限なら色変える
execute if score $item.id iv = $item.id.max iv if score $button.interval.left iv matches 1 run data merge entity @n[type=text_display,tag=iv.text.back] {text:{"text":"Back",color:"red"}}

#下限ならサウンドを鳴らす、意味ないけどreturn 負荷が変わると願う
