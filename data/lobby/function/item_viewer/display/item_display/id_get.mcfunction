...
$execute positioned -20 -1 -7 run data modify storage bf:iv name.fallback set string block ~-$(offset) ~ ~ Items[{Slot:$(slot)b}].id 10
...
#通常アイテムを表示させたあと何故か追加アイテムに切り替えられなくなるバグの対策、name.textにname.fallbackを入れて、表示させるがtranslateは機能しなくなる
data modify storage bf:iv name.text set string storage bf:iv name.fallback
