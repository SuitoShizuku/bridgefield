...
execute if score $item.id iv matches 8 run data merge storage bf:iv {id:"minecraft:wooden_sword",name:"[攻]木刀",lore:{"text":"近接武器"},cost:4,special_effect:"攻撃者が火傷の場合、相手に火傷を付与する"}
execute if score $item.id iv matches 9 run data merge storage bf:iv {id:"minecraft:leather_helmet",name:"[護]皮の帽子",lore:{"text":"装備アイテム"},defence:1}
execute if score $item.id iv matches 10 run data merge storage bf:iv {id:"minecraft:leather_chestplate",name:"[護]皮のジャケット",lore:{"text":"装備アイテム"},defence:3}
