...
# ステータス表示
execute as @a run title @s actionbar [{translate:"bf.actionbar.status",fallback:"§c体力値: %1$s | §9経験値: %2$s | §a資源値: %3$s",with:[{score:{name:"@s",objective:"status.hp"},color:red},{score:{name:"@s",objective:"status.mp"},color:blue},{score:{name:"@s",objective:"status.build"},color:green}]}]
