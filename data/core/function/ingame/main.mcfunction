...
execute as @a run title @s actionbar [{translate:"bf.status.actionbar",fallback:"§a体力値: %1$s | 経験値: %2$s | 資源値: %3$s",with:[{score:{name:"@s",objective:"status.hp"},color:red},{score:{name:"@s",objective:"status.mp"},color:blue},{score:{name:"@s",objective:"status.build"},color:green}]}]