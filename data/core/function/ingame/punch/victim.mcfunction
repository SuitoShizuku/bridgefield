...
execute if items entity @p[tag=punch.attacker] container.* *[custom_data~{"artifactId":"glow_ink_sac"}] unless score @s blindness matches 3.. run scoreboard players set $temp blindness 2
execute if items entity @p[tag=punch.attacker] weapon.offhand *[custom_data~{"artifactId":"glow_ink_sac"}] unless score @s blindness matches 3.. run scoreboard players set $temp blindness 2
