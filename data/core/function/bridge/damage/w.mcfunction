advancement revoke @s only core:bridge_damage_w
...
# 橋が存在すれば耐久値減算
execute at @n[type=interaction,tag=game.interact.bridge,tag=west,nbt={attack:{}}] positioned ~-3 ~4 ~ run function core:bridge/damage/ with entity @s SelectedItem.components."minecraft:custom_data"
# 初期化
