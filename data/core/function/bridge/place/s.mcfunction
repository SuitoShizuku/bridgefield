...
execute if entity @s[tag=placed] at @n[type=interaction,tag=game.interact.bridge,tag=south,nbt={interaction:{}}] positioned ~2 ~-5 ~1 run function core:bridge/place/ with entity @p[tag=placed] SelectedItem.components."minecraft:custom_data"